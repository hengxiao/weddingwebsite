...
a.baseZIndex+100).width(g.width).height(g.height);p.trigger("startSpinning").fadeIn(300);c.show();a.popupIsFixed?(c.width(a.popupWidth).height(a.popupHeight).css("margin-left",-1*(c.innerWidth()/2)+"px").css("margin-top",-1*(c.innerHeight()/2)+"px"),f.on('load',function(){f.off("load");p.hide().trigger("stopSpinning");k.trigger("update",[g.captionText]).fadeIn(a.fadeSpeed);r.fadeIn(a.fadeSpeed);s.css("text-indent",0).hide().fadeIn(a.fadeSpeed,function(){m=!1});t=b;u.fadeIn(a.fadeSpeed)})):f.on('load',function(){f.off("load");
